\documentclass[11pt]{paper}

\input{insbox}
\usepackage[x11names]{xcolor}
\usepackage{lineno}

\usepackage{lib/utt-report-template}
\usepackage{lipsum}

\author{Tamas Spisak}
\date{2020}
\newcommand*\mytitle{\color{pniblue}\textbf{The Users' Manual}}

\newcommand*\mysubtitle{\color{pniblue} \textbf{A}nonymization with \\
\textbf{L}imesurvey integration and \\
\textbf{II}-factor \\
\textbf{A}uthentication for \\
\textbf{S}cientific research}
%\emph{ - dedicated for SFB289 -}

\newcommand*\myshorttitle{ALIIAS: The Users' Manual}
\newcommand*\version{1.0}
\title{\mytitle} % Max 144 chars

\schooltutor{}
\semester{}
\branch{}
\abstracttext{
\input{parts/00-Abstract}
}

\setcounter{tocdepth}{4}

\begin{document}

    %\setlength{\parskip}{0mm}
    %\linenumbers
    \titlespacing*{\section}
    {0pt}{1cm plus 2cm minus 0cm}{0.2cm plus 2cm}
    \titlespacing*{\subsection}
    {0pt}{0.2cm plus 2cm minus 0cm}{0.2cm plus 2cm}

    \frontpage
    
    
    \setlength{\parskip}{1mm}
    
    \pagenumbering{arabic}
    \setcounter{page}{2}
    
    \tableofcontents
    \addtocontents{toc}{~\hfill\textbf{\color{pniblue}Page}\par}
    \thispagestyle{fancy}
    \pagebreak
    
    \begin{large}
    
    \input{parts/01-workflow}
    \input{parts/02-usecases}
    \input{parts/03-installation}
    \input{docs/parts/04-setting_up_ls}
    \input{parts/05-pseudonymization_sop}
    \input{docs/parts/06-barcode_printing}
    \input{docs/parts/07-reidentification}
    \input{docs/parts/08-implementation-details}
    \input{docs/parts/09-faq}
    
    \end{large}
    %\nocite{*}
    \bibliographystyle{apalike}
    \footnotesize
    {\linespread{0}\selectfont\bibliography{references}}
    
    \input{docs/parts/10-printer_set-up}
    \input{docs/parts/11-cheetsheet}
        

    
\end{document}
